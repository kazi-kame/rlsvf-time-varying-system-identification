...
$$
\mathbf{K}(k) =
\frac{\mathbf{P}(k-1)\mathbf{x}(k)}
{\rho + \mathbf{x}^T(k)\mathbf{P}(k-1)\mathbf{x}(k)}
$$

$$
\mathbf{w}(k) =
\mathbf{w}(k-1) + \mathbf{K}(k)e(k)
$$
...
\left[
\mathbf{P}(k-1) -
\mathbf{K}(k)\mathbf{x}^T(k)\mathbf{P}(k-1)
\right]
$$
...
- lengthens memory in steady state  
...
10\log_{10}
\left(
\frac{\lVert \mathbf{w}(k) - \mathbf{w}_{\text{true}}(k) \rVert^2}
{\lVert \mathbf{w}_{\text{true}}(k) \rVert^2}
\right)
$$
...
- $\rho(k)$ decreases when parameters change  
